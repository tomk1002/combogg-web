...
| [06_deploy_plan.md](./06_deploy_plan.md) | 웹 배포 계획 (POC) | 팀원 B |

## 읽는 순서

1. **모두**: `01_overview.md` 먼저
2. **팀원 A (앱)**: `02_desktop_app.md` → `04_shared_spec.md`
3. **팀원 B (웹)**: `03_web.md` → `04_shared_spec.md` → `06_deploy_plan.md`
...
- OAuth 토큰 흐름
- 커스텀 URL 스킴 (`combo://`)
- input category 정의 (LoL의 입력 종류)

## 변경 관리

이 문서들은 CLAUDE.md에서 `@docs/...` 형태로 참조됩니다.

- 명세 변경 시 양 팀(웹·앱)에 즉시 공유
- API 명세 변경은 PR 라벨에 `breaking-spec` 추가
- 문서 변경만의 PR도 코드 변경 PR과 동일하게 리뷰
